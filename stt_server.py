...
client_recently_disconnected = threading.Event()
...
def text_detected(text, loop):
    global prev_text, global_args, recorder # Make sure recorder and global_args are available

    _processed_text = preprocess_text(text) #
    _hard_break_occurred = False

    if silence_timing: #
        # Local helper functions for readability within this callback
        def ends_with_ellipsis(s: str): #
            if s.endswith("..."): return True #
            if len(s) > 1 and s[:-1].endswith("..."): return True #
            return False #

        def sentence_end(s: str): #
            sentence_end_marks = ['.', '!', '?', '。'] #
            if s and s[-1] in sentence_end_marks: return True #
            return False #

        if ends_with_ellipsis(_processed_text): #
            recorder.post_speech_silence_duration = global_args.mid_sentence_detection_pause #
        elif sentence_end(_processed_text) and sentence_end(prev_text) and not ends_with_ellipsis(prev_text): #
        ...
        text_time_deque.append((current_time, _processed_text)) #
        ...
        if len(text_time_deque) >= hard_break_even_on_background_noise_min_texts: #
            texts_in_deque = [t[1] for t in text_time_deque] #
            first_text_in_deque = texts_in_deque[0] #
            last_text_in_deque = texts_in_deque[-1] #
            similarity = SequenceMatcher(None, first_text_in_deque, last_text_in_deque).ratio() #

            if similarity > hard_break_even_on_background_noise_min_similarity and \
               len(first_text_in_deque) > hard_break_even_on_background_noise_min_chars: #
                if recorder_ready.is_set() and recorder:
                    recorder.stop() #
                    recorder.clear_audio_queue() #
                prev_text = "" #
                _hard_break_occurred = True
    # Crucially, update prev_text based on whether a hard break occurred,
    # This ensures that even if we suppress sending, the recorder's context (prev_text for silence_timing) is maintained.
    if not _hard_break_occurred:
        prev_text = _processed_text #
    # If _hard_break_occurred, prev_text is already "", which is the correct state after a hard break.

    # Now check the flag before queueing the message
    if client_recently_disconnected.is_set():
        timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3] #
        print(f"  [{timestamp}] {bcolors.WARNING}text_detected: Suppressing stale realtime text ('{_processed_text}') due to recent disconnect.{bcolors.ENDC}")
        return # Suppress sending

    # If not suppressed, proceed to send and log
    message = json.dumps({'type': 'realtime', 'text': _processed_text}) #
    asyncio.run_coroutine_threadsafe(audio_queue.put(message), loop) #

    timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3] #
    if extended_logging: #
        print(f"  [{timestamp}] Realtime text: {bcolors.OKCYAN}{_processed_text}{bcolors.ENDC}\n", flush=True, end="") #
    else: #
        print(f"\r[{timestamp}] {bcolors.OKCYAN}{_processed_text}{bcolors.ENDC}", flush=True, end='') #


def on_recording_start(loop):
    global is_tts_active, recorder
    message = json.dumps({
        'type': 'recording_start'
    })
    asyncio.run_coroutine_threadsafe(audio_queue.put(message), loop)
    # print tts_active state
    print(f"{bcolors.OKGREEN}TTS active: {is_tts_active}{bcolors.ENDC}")
    if is_tts_active:
        print(f"{bcolors.WARNING}INTERRUPTION: User started speaking while TTS is active.{bcolors.ENDC}")

        # Signal Unity about the interruption
        interruption_payload = {
            'type': 'interruption_detected'
            # You could add more context here if needed, e.g., a timestamp
        }
        interruption_message = json.dumps(interruption_payload)
        asyncio.run_coroutine_threadsafe(audio_queue.put(interruption_message), loop)

        # Reset RealtimeSTT's internal state for the current utterance.
        # This clears any buffered audio and stops current processing,
        # ensuring the STT starts fresh with the interrupting speech.
        if recorder:
            print(f"{bcolors.WARNING}Aborting current STT processing due to interruption.{bcolors.ENDC}")
            recorder.abort() 
...
def parse_arguments():
    ...
    parser.add_argument('--enable_realtime_transcription', action='store_true', default=False,
...
def _recorder_thread(loop):
    global recorder, stop_recorder, prev_text, global_args # Ensure access
    ...
    def process_text(full_sentence): # This is the callback for recorder.text()
        global prev_text # Ensure this is the global prev_text shared with text_detected

        # Reset prev_text regardless, as per original logic for full sentences
        prev_text = "" #

        if client_recently_disconnected.is_set():
            timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3] #
            print(f"  [{timestamp}] {bcolors.WARNING}process_text: Suppressing stale full sentence ('{full_sentence}') due to recent disconnect.{bcolors.ENDC}")
            return # Don't queue or send to client

        _processed_full_sentence = preprocess_text(full_sentence) #
        message = json.dumps({ #
            'type': 'fullSentence', #
            'text': _processed_full_sentence #
        }) #
        asyncio.run_coroutine_threadsafe(audio_queue.put(message), loop) #

        timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3] #
        if extended_logging: #
            print(f"  [{timestamp}] Full text: {bcolors.BOLD}Sentence:{bcolors.ENDC} {bcolors.OKGREEN}{_processed_full_sentence}{bcolors.ENDC}\n", flush=True, end="") #
        else: #
            print(f"\r[{timestamp}] {bcolors.BOLD}Sentence:{bcolors.ENDC} {bcolors.OKGREEN}{_processed_full_sentence}{bcolors.ENDC}\n") #
    try:
        while not stop_recorder: #
            if recorder_ready.is_set() and recorder:
                 recorder.text(process_text) #
            else:
                time.sleep(0.1) # Wait if recorder not ready
        ...
        async for message in websocket:
            ...
            if isinstance(message, str):
                ...
                try:
                    ...
                    elif command == "set_tts_state": # New command handler
                        tts_state = command_data.get("active", False)
                        is_tts_active = bool(tts_state)
                        timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                        print(f"  [{timestamp}] {bcolors.OKGREEN}TTS active state set to: {bcolors.OKBLUE}{is_tts_active}{bcolors.ENDC}")
                        await websocket.send(json.dumps({"status": "success", "message": f"TTS state set to {is_tts_active}"}))
                    else:
...
async def data_handler(websocket):
    global writechunks, wav_file, prev_text, audio_queue # Ensure audio_queue is accessible
    print(f"{bcolors.OKGREEN}Data client connected{bcolors.ENDC}")
    data_connections.add(websocket)

    # If a client connects, assume it's a new, valid session. Clear the flag.
    if client_recently_disconnected.is_set():
        print(f"{bcolors.OKBLUE}New data client connected. Clearing 'recently_disconnected' flag.{bcolors.ENDC}")
        client_recently_disconnected.clear()
    ...
    try:
        while True:
            ...
            if isinstance(message, bytes):
                ...
                # else:
                #     # recorder.feed_audio(chunk)
                #     if recorder_ready.is_set() and recorder: # Check if recorder is ready
                #         recorder.feed_audio(chunk)
            else:
        ...
        # Only set the flag and perform aggressive cleanup if this was the *last* data connection.
        if not data_connections:
            print(f"{bcolors.WARNING}Last data client disconnected. Setting 'recently_disconnected' flag.{bcolors.ENDC}")
            client_recently_disconnected.set() # Signal that a disconnect happened

            if recorder_ready.is_set() and recorder:
                print(f"{bcolors.WARNING}Aborting STT processing and clearing recorder's internal audio queue.{bcolors.ENDC}")
                recorder.abort() # Attempt to stop current STT work
                recorder.clear_audio_queue() # Clear recorder's own audio buffer
            
            global prev_text # Ensure we're modifying the global
            prev_text = "" # Reset prev_text state

            # Also clear any messages that might have been queued for broadcast by the recorder
            # thread *before* it could check the 'client_recently_disconnected' flag.
            print(f"{bcolors.WARNING}Clearing broadcast audio_queue to remove potentially stale messages.{bcolors.ENDC}")
            while not audio_queue.empty(): #
                try:
                    audio_queue.get_nowait() #
                except asyncio.QueueEmpty: #
                    break
            print(f"{bcolors.WARNING}Broadcast audio_queue cleared.{bcolors.ENDC}")
        else:
            print(f"{bcolors.OKBLUE}Data client disconnected. Remaining connections: {len(data_connections)}{bcolors.ENDC}")
