    ...
    public class LLMClient : MonoBehaviour
    {
        ...
        protected virtual async Task CheckCaller(bool checkConnection = true)
        {
            ...
            if (remote && checkConnection)
            {
                ...
                {
                    if (await llmClient.IsServerAliveAsync()) break;
                    await Task.Yield();
                }
            }
        }
        ...
        protected virtual async Task SetupCallerObject()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!connectionHost.StartsWith("http://") && !connectionHost.StartsWith("https://"))
                    {
                        connectionHost = "http://" + connectionHost;
                    }
                    llmClient = new UndreamAI.LlamaLib.LLMClient(connectionHost, port, APIKey, numRetries);
                }
            }
            catch (Exception ex)
            {
        ...
        public virtual void SetGrammar(string grammarString)
        {
            ...
            if (caller is UndreamAI.LlamaLib.LLMClient client) client.SetGrammar(_grammar);
            else if (caller is UndreamAI.LlamaLib.LLMAgent agent) agent.SetGrammar(_grammar);
            else caller?.SetGrammar(_grammar);
        }
        ...
        protected virtual void SetCompletionParameters()
        {
            ...
            {
                var caller = GetCaller();
                if (caller is UndreamAI.LlamaLib.LLMClient client) client.SetCompletionParameters(parameters);
                else if (caller is UndreamAI.LlamaLib.LLMAgent agent) agent.SetCompletionParameters(parameters);
                else caller?.SetCompletionParameters(parameters);
